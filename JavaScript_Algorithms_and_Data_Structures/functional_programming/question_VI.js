 ...
 * Hint: using something like const newArr = arrVar, where arrVar is an array
 * will simply create a reference to the existing variable and not a copy.
 * So changing a value in newArr would change the value in arrVar.
 *
 * Declare function parameters - any computation inside a function depends only
 ...
 *
 * Q: Rewrite the code so the global array bookList is not changed inside either function.
 * The add function should add the given bookName to the end of the array passed to it
 * and return a new array (list). The remove function should remove the given
...
function add(bookArr, bookName) {
    const unRefBookArr = [...bookArr];
    unRefBookArr.push(bookName);
    return unRefBookArr;
...
function remove(bookArr, bookName) {
    const unRefBookArr = [...bookArr];
    const book_index = unRefBookArr.indexOf(bookName);
    if (book_index >= 0) {
        unRefBookArr.splice(book_index, 1);
        return unRefBookArr;
        // Change code above this line
    }
}
