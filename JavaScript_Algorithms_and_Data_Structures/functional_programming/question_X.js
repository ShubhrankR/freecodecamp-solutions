 ...
 * object, array). This is achieved via a callback function that is
 ...
 * callback function from the previous iteration, the second is the
 * current element being processed, the third is the index of that
 ...
 * the second iteration gets passed the first element of the array as
 * the accumulator.
 *
 * Q: The variable watchList holds an array of objects with information
 * on several movies. Use reduce to find the average IMDB rating of the
...
function getRating(watchList) {
    ...
    let averageRating, filteredArr, mapArr, aggreagatedSum;
    filteredArr = watchList.filter(
        (movie) => movie.Director === "Christopher Nolan"
    );
    mapArr = filteredArr.map((m) => {
        return Number(m.imdbRating);
    });
    // let reducer = (previousVal, currentVal) => (previousVal.rating + currentVal.rating);
    aggreagatedSum = mapArr.reduce((pv, cv) => pv + cv);
    averageRating = aggreagatedSum / mapArr.length;
